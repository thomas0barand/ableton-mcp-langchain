...
class AbletonMCP(ControlSurface):
    ...
    def _set_device_parameter(self, track_index, device_index, parameter_index, value):
        """Set a device parameter using normalized values (0.0 to 1.0)"""
        try:
            if track_index < 0 or track_index >= len(self._song.tracks):
                raise IndexError("Track index out of range")
    ...
    def _batch_set_device_parameters(self, track_index, device_index, parameter_indices, values):
        """Batch set device parameters using normalized values (0.0 to 1.0)"""
        try:
            if track_index < 0 or track_index >= len(self._song.tracks):
                raise IndexError("Track index out of range")
            ...
            if len(parameter_indices) != len(values):
                raise ValueError("Number of parameter indices must match number of values")
            
            results = []
            for i in range(len(parameter_indices)):
                param_index = parameter_indices[i]
                normalized_value = values[i]
                parameter = device.parameters[param_index]
                
                # Convert normalized value (0.0 to 1.0) to actual parameter value
                if parameter.max != parameter.min:
                    actual_value = parameter.min + (normalized_value * (parameter.max - parameter.min))
                else:
                    actual_value = parameter.min
